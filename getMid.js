...
const apiMid = 'http://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst'
...
async function testMid({nx,ny}){
    const {base_date,base_time}=baseTimeMid()
    const apiKey=process.env.APIKEY

    const url = `${apiMid}?serviceKey=${apiKey}&numOfRows=${numOfRows}&pageNo=${pageNo}&dataType=${dataType}&base_date=${base_date}&base_time=${base_time}&nx=${nx}&ny=${ny}`
    try{
